...
현재 네 개인 스킬에는 `skills.sh.json`을 추가하지 않는다. 이 파일은 skills.sh 검색 등록의 필수 조건이 아니며, 스킬 수와 카테고리가 늘어 별도 분류 metadata가 실제로 필요할 때만 다시 검토한다.
...
| runtime names | `sonsu-skills:<frontmatter.name>` | skill selector와 명시적 invocation | 현재는 `sonsu-skills:architecture-red-team`, `sonsu-skills:to-commit`, `sonsu-skills:to-pr`, `sonsu-skills:to-skill`만 노출한다. |
...
flowchart LR
    ...
    E --> J["sonsu-skills:architecture-red-team"]
...
- plugin cache에 비활성 저장소 파일이 함께 복사되더라도 runtime에는 `sonsu-skills:architecture-red-team`, `sonsu-skills:to-commit`, `sonsu-skills:to-pr`, `sonsu-skills:to-skill`만 plugin skill로 노출된다.
