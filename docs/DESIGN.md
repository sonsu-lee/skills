...
이 저장소는 별도 플러그인 저장소를 만들지 않고 하나의 공개 번들 플러그인 `sonsu-skills`를 Codex에 제공한다. 플러그인은 `skills/`에 있는 직접 작성·관리 스킬만 포함하며 외부 catalog 항목을 번들에 넣지 않는다.
...
2. Codex native plugin은 개인 스킬 전체를 `sonsu-skills`라는 관리형 번들로 설치하고 업데이트한다. 일부 스킬만 선택하지 않으며 사용자가 로컬 사본을 직접 관리하지 않는 채널이다. Claude Code에서는 이 채널을 제공하지 않는다.
...
.agents/plugins/marketplace.json     # one sonsu-skills entry
catalog/                             # external metadata only
scripts/                             # profile installer
```

Codex manifest는 `./skills/`를 가리킨다. 테스트에서 manifest가 노출하는 집합이 `skills/*/SKILL.md` 집합과 정확히 같은지 확인한다.

plugin manifest에는 명시적 semver를 기록하고 plugin 변경과 함께 수동으로 올린다. marketplace에는 `sonsu-skills` 하나만 노출한다. 초기에는 자동 release workflow, 자동 version bump와 별도 package registry를 만들지 않는다.
...
profile CLI가 개인 스킬을 조합할 때는 이 저장소의 `skills/`를 `npx skills` 설치 소스로 사용한다. 외부 항목은 `catalog/`의 upstream source나 host provider를 사용한다. CLI를 통한 선택 설치와 native plugin을 통한 전체 번들은 독립 채널로 유지한다.
...
- JSON schema, 경로, 노출 집합, semver 형식과 외부 catalog 격리를 테스트한다.
...
2. `feat(plugin): add sonsu-skills Codex bundle`
   ...
   - JSON 구조, 참조 경로, owned skill 노출 집합, 명시적 semver, 단일 marketplace entry와 외부 catalog 미노출을 자동 검증한다.
...
- Codex repo marketplace에는 `sonsu-skills` 하나만 노출된다.
- plugin에는 외부 catalog 항목, custom agent, hook, MCP server, app, connector, model routing 설정과 asset이 포함되지 않는다.
