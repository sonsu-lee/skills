...
상태: 기본 설계 승인, Codex plugin 배포는 구현 전
...
현재 세 개인 스킬에는 `skills.sh.json`을 추가하지 않는다. 이 파일은 skills.sh 검색 등록의 필수 조건이 아니며, 스킬 수와 카테고리가 늘어 별도 분류 metadata가 실제로 필요할 때만 다시 검토한다.

### 현재 구현 상태

| 영역 | 상태 | 현재 계약 |
| --- | --- | --- |
| owned skills | 구현됨 | `skills/`의 세 스킬이 배포 SSOT다. |
| 외부 catalog와 profile CLI | 구현됨 | 현재 CLI는 catalog의 외부 항목만 조합한다. |
| Codex `sonsu-skills` plugin | 설계됨, 미구현 | manifest와 marketplace가 없으므로 아직 설치할 수 없다. |
...
| runtime names | `sonsu-skills:<frontmatter.name>` | skill selector와 명시적 invocation | 현재는 `sonsu-skills:to-commit`, `sonsu-skills:to-pr`, `sonsu-skills:to-skill`만 노출한다. |
| activation | 설치된 plugin의 enabled 상태 | Codex runtime | enabled일 때 세 스킬 전체, disabled일 때 어느 것도 노출하지 않는다. |
...
plugin manifest와 marketplace의 초기 계약은 다음 값으로 맞춘다. 구현 시 공식 schema가 달라졌으면 최신 계약에 맞춰 함께 수정하되 위 경계는 유지한다.
...
이 probe는 현재 접근이 가능한지 확인한 설계 검증이며 실제 repository plugin 배포를 대신하지 않는다. 구현 PR에서는 같은 검증을 재현 가능한 test로 만들고 Codex 앱의 새 작업에서 discovery와 invocation을 다시 확인한다.
...
- 선택 설치와 관리형 전체 번들의 운영 사례: <https://github.com/mattpocock/skills#install-as-a-claude-code-plugin>
...
- Codex 앱의 새 작업에서 세 namespaced skill의 발견과 명시적 호출을 수동 smoke test한다.
...
현재 설치 대상으로 등록된 항목은 `catalog/skills.json`이 결정한다. 연구 보고서나 논의에만 등장하는 외부 프로젝트와 스킬은 사용자가 승인해 catalog에 추가하기 전까지 미등록 후보다. Matt Pocock 계열을 추가 조사할 때는 `domain-modeling`, `grill-me`, `grill-with-docs`를 우선 검토하고 중복 항목은 제외할 수 있다.
...
- plugin cache에 비활성 저장소 파일이 함께 복사되더라도 runtime에는 `sonsu-skills:to-commit`, `sonsu-skills:to-pr`, `sonsu-skills:to-skill`만 plugin skill로 노출된다.
...
- 현재 세 개인 스킬에는 불필요한 `skills.sh.json`이 추가되지 않는다.
