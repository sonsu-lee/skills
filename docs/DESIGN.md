...
이 저장소 자체는 plugin package, custom agent 정의, model routing 설정과 marketplace를 배포하지 않는다. 외부 공식 플러그인은 catalog provider와 설치 안내로만 다루고, 소비 프로젝트에서 host 전용 packaging이 명시적으로 필요하면 portable skill 작성과 분리된 host adapter 또는 별도 저장소 작업으로 취급한다.
...
Plugin packaging은 이 저장소의 기본 산출물이 아니다. 소비 프로젝트에서 사용자가 배포를 명시적으로 요청하면 portable skill 작성과 분리된 host 전용 작업으로 보고, 해당 호스트의 현재 공식 creator/provider와 계약을 사용한다. 호스트 내장 creator 또는 공식 plugin provider는 단일 호스트 전용 스킬과 최신 제품 기능을 위한 fallback으로 유지한다.
...
- plugin package, custom agent, model routing과 marketplace는 이 저장소에 추가하지 않고 실제 필요가 생기면 별도 host adapter 또는 저장소에서 다룬다.
...
- 이 저장소에는 plugin package, custom agent, model routing 설정과 marketplace가 기본 산출물로 생성되지 않는다.
