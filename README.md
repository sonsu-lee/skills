...
`recommend-skill`과 `review-dev-resume`를 제외한 전문 스킬은 목표가 자연어 요청에 명확하면 이름을 직접 쓰지 않아도 자동으로 선택될 수 있습니다. 자동 선택은 파일, Git 또는 외부 시스템 변경 권한이 아니며, 각 스킬은 사용자가 요청한 동작과 내부 승인 gate 안에서만 상태를 바꿉니다.
