...
설정 조회 실패에 credential store, network deny 또는 sandbox 격리 징후가 있으면 `../../references/host-auth-and-signing.md`를 읽고 허용된 읽기 전용 외부 진단을 최대 한 번 적용한다. 그래도 확인하지 못하거나 일반적인 API 접근 제한이면 위의 `unverified` 경로를 사용한다.
...
PR 제목을 만들거나 base/head diff와 commit history를 판정해야 하면 먼저 `../../references/change-policy.md`를 읽고 저장소 규칙과 Conventional Commits 정책을 적용한다. 공통 정책의 squash 기본값은 확인한 merge mode에 맞춰 적용한다.
