---
name: maintain-domain-docs
description: "제품 도메인의 정본 용어, 개념, 역할, 상태, 전이, 사건, 비즈니스 규칙을 근거와 함께 생성·정비한다. 도메인 문서, glossary, ubiquitous language, bounded context, 상태 모델, 업무 규칙, 용어 충돌·동의어·폐기 요청에 사용한다. 특정 기능의 문제·범위·수용 기준을 쓰는 PRD 작업이나 선택 이유를 남기는 결정 기록, 코드·스키마 구현 요청에는 사용하지 않는다."
---
...
- 왜 한 제품·정책·아키텍처 선택을 했는지에 대한 장기 rationale
- 코드, 테스트, 데이터 마이그레이션 또는 API 구현
- `openwiki/` 아래의 생성 결과

다른 문서에서 지속적인 도메인 지식이 발견되면 이 스킬로 승격할 수 있다. 반대로 PRD 또는 Decision Record 변경이 필요하면 영향과 후보만 보고하고 사용자 승인 없이 수정하지 않는다.
...
- write authorization, semantic approval, document ownership을 구분한다. 사용자의 쓰기 요청만으로 stable canonical state를 부여하지 않으며, 이 스킬은 어떤 경우에도 PRD나 Decision Record를 직접 수정하지 않는다.
...
사용자가 문서 변경을 요청하지 않았다면 분석과 proposed diff만 제공한다. 기능 요구사항이나 결정 rationale가 목적이면 정확한 companion skill로 라우팅한다.
...
- 각 source와 authority
- 달라지는 의미와 유지되는 의미
- 영향을 받는 PRD, Decision Record, domain entry, 코드·테스트·schema 링크
- 필요한 owner 또는 승인
- 대안과 변경하지 않을 때의 결과
...
- 검증 결과
- PRD 또는 Decision Record companion skill로 넘길 후보
