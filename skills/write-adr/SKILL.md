---
name: write-adr
description: "중요한 아키텍처·기술 결정을 실제로 검토한 선택지, 선택 이유, 결과, 다시 볼 조건과 함께 ADR로 작성하거나 고친다. ADR 작성, architecture decision, 기술 선택 기록, 기존 ADR 검토·대체·폐기 요청에 사용한다. 제품·사업·정책 결정, PRD 작성, 도메인 문서, 아직 결론 없는 기술 비교, 일반 코드 작업에는 사용하지 않는다."
---
...
PRD나 Domain Doc에서 아키텍처 결정 후보를 찾을 수 있지만 원래 문서의 내용을 복제하지 않는다. 필요한 context와 상대 링크만 둔다. 제품·사업·정책 결정은 ADR로 바꾸지 않는다. 문제·사용자·결과·범위·핵심 규칙을 더 결정해야 하면 `product-discovery`, 이 컨텍스트가 준비됐고 PRD 산출물을 원하면 `to-prd`로 라우팅한다.
...
한 문장에 여러 결정이 있으면 각각의 독립 실패 가능성과 승인 주체를 보고 분리한다. 결론 없는 options 비교라면 분석은 도울 수 있지만 ADR을 accepted로 쓰지 않는다.
