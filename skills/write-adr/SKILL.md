...
이 호환 진입점은 단독 선택 설치를 지원하지 않는다. 독립 스킬로 설치할 때는 `architecture-decisions`와 `to-adr`를 같은 범위에 함께 설치한다. 전체 플러그인 설치에는 세 스킬이 모두 포함된다.
...
필요한 companion 스킬을 사용할 수 없으면 동작을 복제하거나 추측하지 않는다. 누락된 `architecture-decisions` 또는 `to-adr`를 정확히 밝히고, 필요한 조합을 함께 설치하거나 전체 플러그인을 설치하라고 안내한다.
