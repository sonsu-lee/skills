# Write ADR Evaluation Rubric

## Protocol

- 공통 격리·stepwise·security 규칙은 `../../../evals/product-docs/protocol.md`, 공통 assertion은 `../../../evals/product-docs/README.md`를 따른다.
- Routing 평가는 manifest에서 발견 가능한 모든 installable skill의 `name`과 `description`을 함께 제공한다.
...
3. frontmatter의 `type: ADR`, `id: ADR-*`, `kind: architecture`, `title`, document `status`, `workflow_status`, 날짜, decision maker, status events, sources, typed relations, lifecycle links, visibility·publication을 검사한다. 본문에 별도 status event 정본을 복제하면 실패한다.
...
11. 같은 입력을 두 번 실행해 중복 ADR이나 새 ID가 생기지 않는지 확인한다.
