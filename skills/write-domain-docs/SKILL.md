...
사용자가 문서 변경을 요청하지 않았다면 분석과 proposed diff만 제공한다. 기능 요구사항이면 `create-prd`, 아키텍처·기술 선택의 이유를 남기려는 요청이면 `write-adr`로 라우팅한다. 제품·사업·정책 결정 기록은 이 스킬이나 `write-adr`로 보내지 않는다.
...
- 검증 결과
- PRD 또는 `write-adr`로 넘길 아키텍처 결정 후보
