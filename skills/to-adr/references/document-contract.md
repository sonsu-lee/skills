...
2. 기존 PRD, 도메인 문서, ADR, 템플릿, 번호 체계, 링크 관례를 검색한다.
...
| ADR | `docs/adr/` | `ADR-0001-recovery-channel.md` |
...
| ADR | `draft` | `proposed` | 승인되지 않은 아키텍처 제안 |
| ADR | `stable` | `accepted` 또는 `rejected` | 아키텍처 결정 또는 기각이라는 역사적 사건이 확인됨 |
| ADR | `deprecated` | `deprecated` 또는 `superseded` | 더 이상 현재 결정이 아니거나 대체됨 |
...
PRD ──발견──> Domain Promotion Candidate ──승인──> Domain Doc
 │
 └──아키텍처 선택 발견──> ADR Candidate ──승인──> ADR
```

- PRD는 특정 제품 변화의 합의를 소유한다.
- Domain Doc은 여러 기능에서 재사용되는 업무 의미와 규칙을 소유한다.
- ADR은 아키텍처·기술 선택의 이유와 결과를 소유한다. 제품·사업·정책 결정은 소유하지 않는다.
...
- `docs/` 아래의 PRD, Domain Doc, ADR이 정본이다.
