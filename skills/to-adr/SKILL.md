---
name: to-adr
description: "이미 내려졌거나 구체적인 proposal로 준비된 중요한 아키텍처·기술 결정을 실제 선택지, 이유, 결과와 재검토 조건을 갖춘 ADR로 작성하거나 고친다. 사용자가 ADR 기록·대체·폐기를 요청할 때 사용한다. 제품·사업·정책 결정, PRD, 도메인 문서, 아직 결론 없는 기술 비교와 일반 코드 작업에는 사용하지 않는다."
---
...
ADR 작성·갱신 의도가 요청에서 명확하면 스킬 이름 없이도 활성화한다. 자동 활성화는 파일 쓰기, 승인 상태 변경이나 외부 게시 권한이 아니다.
