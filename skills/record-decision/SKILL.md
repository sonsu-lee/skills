...
description: "제품·정책·아키텍처의 중요한 결정을 맥락, 실제 고려한 선택지, 근거, 결과, 재검토 조건과 함께 Decision Record로 생성·갱신한다. ADR, decision record, 의사결정 로그, 선택 이유 보존, 결정 대체·폐기 요청에 사용한다. 아직 결론 없는 브레인스토밍, PRD 요구사항 작성, 도메인 정의 정비, 쉽게 되돌릴 수 있는 국소 구현 가이드·코드 절차에는 사용하지 않는다."
---
...
2. [Decision quality bar](references/decision-quality-bar.md)를 읽는다.
3. 새 문서를 쓸 때는 [Decision Record template](assets/decision-record-template.md)을 출발점으로 삼고 관련 없는 섹션과 모든 자리표시자를 제거한다.

## 이 스킬의 경계

소유하는 것:

- 하나의 중요한 제품, 정책 또는 아키텍처 결정
...
PRD나 Domain Doc에서 결정 후보를 승격할 수 있지만 원래 문서의 내용을 복제하지 않는다. 필요한 context와 상대 링크만 둔다.

## 기록 가치 판정

다음 중 하나 이상이면 Decision Record 후보가 될 가능성이 높다.

- 되돌리기 어렵거나 변경 비용이 크다.
- 여러 팀, 서비스, 사용자 흐름 또는 제품 영역에 영향을 준다.
- 보안, 개인정보, 규제, 데이터, API 경계, 핵심 품질 속성에 영향을 준다.
- 장기간 유지될 제품 정책이나 플랫폼 제약을 정한다.
...
한 문장에 여러 결정이 있으면 각각의 독립 실패 가능성과 승인 주체를 보고 분리한다. 결론 없는 options 비교라면 분석은 도울 수 있지만 Decision Record를 accepted로 쓰지 않는다.
...
- 정책, 제품, 아키텍처 중 kind가 무엇인가
- 누가 결정할 권한을 갖는가
- 기존 Decision Record와 중복되거나 이미 supersede된 내용은 아닌가
...
### 5. Elicit one missing decision unit

사실과 history는 source에서 먼저 확인한다. 선택과 판단은 decision maker에게 묻는다.
...
제품·아키텍처 trade-off에 추천을 제공해 달라는 요청이 있으면 현재 evidence, constraints, assumptions, confidence를 분리한다. 추천 자체가 결정이나 historical rationale가 되지는 않는다. 사용자의 명시적 선택을 기다린다.
...
1. 새 DR ID를 `draft/proposed`로 만들고 `proposes_to_supersede`에 이전 accepted record를 연결한다. 이전 기록은 계속 `stable/accepted`다.
...
과거 rejected proposal을 나중에 채택하는 것은 원래 rejected history를 supersede하지 않는다. 새 accepted DR이 `revisits`로 연결하고 rejected record는 그대로 유지한다.
...
[Decision quality bar](references/decision-quality-bar.md)를 적용한다.
...
- related PRD·Domain Doc·Decision Record link
...
- 생성 또는 갱신한 Decision Record 경로, kind, `status/workflow_status`
