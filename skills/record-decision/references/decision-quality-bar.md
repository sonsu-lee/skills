...
- 되돌리기 어렵거나 변경 비용이 큰가?
- 여러 팀, 시스템, 제품 영역 또는 장기간 정책에 영향을 주는가?
...
- 안정 ID, `kind: product | policy | architecture`, title, document `status`, decision `workflow_status`
...
- 관련 PRD, Domain Doc, Decision Record, evidence link
...
| superseded | 유효한 새 DR 링크와 양방향 관계 | 기존 본문을 새 결정에 맞춰 재작성 |
...
- 다른 독립 결정이 option 안에 섞이면 별도 DR로 분리한다.
