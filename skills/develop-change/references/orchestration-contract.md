...
- terminal `verify` handoff는 passed/failed/not_run 중 비어 있지 않은 검증 결과를 최소 한 건 남긴다.
...
- effect capability를 가진 route는 현재 effect의 capability·target·scope·basis와 정확히 일치하는 dependent authorization evaluation을 한 건만 가진다. 다른 evaluation은 `side_effect_intent: none`, `dependent_side_effect_count: 0`이어야 한다.
...
- terminal route가 effect capability를 가졌다면 실행 결과를 다시 확인할 수 있는 비어 있지 않은 artifact 식별자를 최소 한 건 남긴다.
