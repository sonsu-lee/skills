...
- 각 full SHA의 parent, author·committer identity와 timestamp, 전체 메시지와 diff를 검사한다. DCO나 허용 identity 정책이 있으면 author·committer metadata를 필수 trailer와 대조한 뒤 range의 누적 diff를 별도로 검사한다.
