...
4. 실행 전후 local Git과 원격 PR 상태를 비교한다. 생성 전 artifact 검토에서는 원격 PR이 없음을 정상 입력으로 취급한다.
5. 기존 PR과 생성 전 PR artifact의 readiness 검토는 이 스킬로 처리하고, uncommitted 변경, standalone commit review, 실제 create/update/merge와 일반 코드 리뷰는 `expected_route`로 보낸다.
...
| `accept_pr_artifacts_without_remote_identity`, `require_artifact_snapshot` | 생성 전 제목·본문 후보는 PR 식별자·원격 check 없이 검토하되 base/head snapshot·전체 diff·artifact를 요구 |
...
- 대상별 필수 identity 또는 artifact·전체 diff·final history 기준의 중요 미확인은 `fail`이며, 비차단 check·최신성 미확인만 있으면 `pass_with_warnings`다. 생성 전 artifact에서 원격 PR 부재만으로 실패시키지 않는다.
...
- 한국어·영어·혼합 PR review와 생성 전 PR artifact readiness 요청에서 이 스킬을 선택한다.
