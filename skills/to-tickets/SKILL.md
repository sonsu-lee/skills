---
name: to-tickets
description: "승인된 PRD, 기술 결정, 구현 계획이나 현재 대화의 준비된 컨텍스트를 의존 관계가 드러나는 실행 가능한 여러 티켓으로 변환한다. 사용자가 작업 분해, implementation tickets, issue plan, tracer-bullet backlog를 만들거나 승인 후 이슈 트래커에 게시해 달라고 요청할 때 사용한다. 제품 요구사항·기술 결정을 새로 만들거나 일정·공수를 근거 없이 추정하는 요청에는 사용하지 않는다."
---
...
- 티켓 분해·게시 의도가 요청에서 명확해야 한다. 스킬 이름을 직접 호출할 필요는 없다.
- 자동 활성화는 이슈 트래커 게시 권한이 아니다.
